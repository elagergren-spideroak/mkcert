...
func (m *mkcert) makeCert(hosts []string) error {
	if m.caKey == nil {
		return errors.New("ERROR: can't create new certificates because the CA key (rootCA-key.pem) is missing")
	}

	priv, err := m.generateKey(false)
	if err != nil {
		return fatalIfErr(err, "failed to generate certificate key")
	}
	...
	if err != nil {
		return fatalIfErr(err, "failed to generate certificate")
	}
	...
	if !m.PKCS12 {
		...
		if err != nil {
			return fatalIfErr(err, "failed to encode certificate key")
		}
		...
		if certFile == keyFile {
			...
			if err != nil {
				return fatalIfErr(err, "failed to save certificate and key")
			}
		} else {
			err = ioutil.WriteFile(certFile, certPEM, 0o644)
			if err != nil {
				return fatalIfErr(err, "failed to save certificate")
			}
			err = ioutil.WriteFile(keyFile, privPEM, 0o600)
			if err != nil {
				return fatalIfErr(err, "failed to save certificate key")
			}
		}
	} else {
		...
		if err != nil {
			return fatalIfErr(err, "failed to generate PKCS#12")
		}
		err = ioutil.WriteFile(p12File, pfxData, 0o644)
		if err != nil {
			return fatalIfErr(err, "failed to save PKCS#12")
		}
	}
...
func randomSerialNumber() (*big.Int, error) {
	...
	if err != nil {
		return nil, fatalIfErr(err, "failed to generate serial number")
	}
...
func (m *mkcert) makeCertFromCSR() error {
	...
	if err != nil {
		return fatalIfErr(err, "failed to read the CSR")
	}
	csrPEM, _ := pem.Decode(csrPEMBytes)
	if csrPEM == nil {
		return errors.New("ERROR: failed to read the CSR: unexpected content")
	}
	if csrPEM.Type != "CERTIFICATE REQUEST" &&
		csrPEM.Type != "NEW CERTIFICATE REQUEST" {
		return errors.New("ERROR: failed to read the CSR: expected CERTIFICATE REQUEST, got " + csrPEM.Type)
	}
	csr, err := x509.ParseCertificateRequest(csrPEM.Bytes)
	if err != nil {
		return fatalIfErr(err, "failed to parse the CSR")
	}
	if err := csr.CheckSignature(); err != nil {
		return fatalIfErr(err, "invalid CSR signature")
	}
...
func (m *mkcert) loadCA() error {
	...
	if err != nil {
		return fatalIfErr(err, "failed to read the CA certificate")
	}
	...
	if err != nil {
		return fatalIfErr(err, "failed to parse the CA certificate")
	}
	...
	if err != nil {
		return fatalIfErr(err, "failed to read the CA key")
	}
	...
	if err != nil {
		return fatalIfErr(err, "failed to parse the CA key")
	}
	return nil
}
...
func (m *mkcert) newCA() error {
	...
	if err != nil {
		return fatalIfErr(err, "failed to generate the CA key")
	}
	pub := priv.(crypto.Signer).Public()

	spkiASN1, err := x509.MarshalPKIXPublicKey(pub)
	if err != nil {
		return fatalIfErr(err, "failed to encode public key")
	}
	...
	if err != nil {
		return fatalIfErr(err, "failed to decode public key")
	}
	...
	if err != nil {
		return fatalIfErr(err, "failed to generate CA certificate")
	}

	privDER, err := x509.MarshalPKCS8PrivateKey(priv)
	if err != nil {
		return fatalIfErr(err, "failed to encode CA key")
	}
	err = ioutil.WriteFile(filepath.Join(m.CAROOT, rootKeyName), pem.EncodeToMemory(
		&pem.Block{Type: "PRIVATE KEY", Bytes: privDER}), 0o400)
	if err != nil {
		return fatalIfErr(err, "failed to save CA key")
	}
